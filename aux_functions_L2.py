...
def thermal_conversion(array, xml_path):
    ...
    """
    Convert Landsat 8 Band 10 digital number (DN) values into absolute temperature (Kelvin).

    This function reads the thermal calibration parameters from the Landsat MTL XML file
    and applies the standard Landsat Level-2 Surface Temperature conversion:

        T = (DN * ML) + AL

    where:
        - DN is the Band 10 pixel value (digital number)
        - ML is the TEMPERATURE_MULT_BAND_ST_B10 coefficient
        - AL is the TEMPERATURE_ADD_BAND_ST_B10 coefficient

    These fields must exist inside the XML file under:
    ...
    ----------
    array : np.ndarray
        2D array of Band 10 DN values (typically read directly from the ST_B10.tif file).

    xml_path : str
        Path to the Landsat MTL XML metadata file.
        The file must contain the temperature conversion coefficients used for L2 processing.

    RETURNS
    -------
    Thermal : np.ndarray
        A float32/float64 array of the same shape as input, containing the absolute
        temperature values in Kelvin.

    NOTES
    -----
    • This function assumes Landsat 8 Level-2 data, where the thermal calibration
      coefficients ML and AL are provided in the XML file.
    • No masking, scaling, or unit conversions beyond the Landsat-provided formula
      are performed here.
    """
